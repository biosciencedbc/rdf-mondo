...
DATA_DIR=/data
FORCE_CONVERT=0
VERSION_FILE="mondo_version.txt"
...
cd ${WORK_DIR}
# versionの確認
version=`curl -s -L "http://www.ebi.ac.uk/ols/api/ontologies/mondo" | jq '.config | .version'`
file_url=`curl -s -L "http://www.ebi.ac.uk/ols/api/ontologies/mondo" | jq '.config | .id' | sed "s/\"//g"` 
